...
Status as of 2026-08-23 (this update adds editable ingredient unit costs, the P&L dashboard (automatic COGS from the existing recipe/BOM data), and the Menu Editing tab, all now live in production, on top of everything from the 2026-08-22 doc sync: menu photos, the 11-milestone order-lifecycle/executive-tier overhaul, a 5-milestone bugfix pass, payroll demo data + system health check, the Generate Payroll bugfix + SMFC-parity payroll UX + PDF payslips + Oishii AI launch, the Inventory Count rebuild + sortable columns, Oishii AI's full-business data expansion, shipment-receiving reactivation + unit cost/expiry tracking, and switching Oishii AI from xAI to Groq + fixing a real rate-limit bug this surfaced). All 4 apps remain live in production. Update this file as work advances.
...
-2. **Editable ingredient unit cost** — new "Unit Cost" column on Inventory Count (executive-only edit, read-only for other roles), backed by a new `PATCH /inventory/{id}` endpoint. Unblocks the P&L dashboard below: costs previously could only be set indirectly via a delivery movement. Only builds the tool -- filling in the real ~69 still-uncosted ingredients is a manual step for the client. Verified live (set/clear a cost, role-gating, zero console errors) and deployed.
-1. **P&L dashboard: automatic COGS from existing recipe/BOM data** — new `/pnl` tab (Today/Week/Month toggle) computing revenue, gross/net profit, food cost %, a cost breakdown (COGS/payroll/utilities/losses), and margin by department. Deliberately reuses `recipe_items`/`bundle_fulfillments`/`ingredients.unit_cost` at report time instead of building new BOM/snapshot tables. Known accepted gaps, surfaced via UI callouts: only 3/73 ingredients are costed so far, unfulfilled bundle sales show ₱0 COGS, no historical cost snapshot (past-period COGS uses today's costs), no utility-bill day-apportionment. Verified live: a controlled sale and a controlled bundle-fulfillment each moved COGS by the exact expected amount, voiding each reversed the delta exactly, payroll cost matched `/attendance/summary` exactly. Deployed and re-verified against production.
0. **Menu Editing: executive-only product/recipe/image CRUD** — the catalog had been read-only since the original xlsx seed. New `/menu-editing` tab lets an executive create/edit/deactivate menu items, edit per-size prices, edit recipe/ingredient components, and upload real product photos (new Supabase Storage bucket `product-images` -- the app had no file storage before this, images were static repo files). "Delete" is a soft-delete via the existing `products.active` flag (hard-deleting would break FK-referenced sales/loss history). Deployed and verified live on production (create/upload/deactivate cycle confirmed through the real UI). Not built: a `description` field (not requested), bulk size reordering, orphaned-Storage-object cleanup on repeat image replacement.
...
| 11 — Menu Editing (executive catalog CRUD) | **Done, live** — product/size/recipe CRUD, real image upload via new Supabase Storage bucket, soft-delete via `active` flag; deployed and verified on production |
| 12 — P&L dashboard (automatic COGS) | **Done, live** — reuses existing recipe_items/bundle_fulfillments/ingredients.unit_cost, no new BOM schema, no snapshot job; deployed and verified on production |
