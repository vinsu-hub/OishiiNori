...
**Digital menu visual redesign + add-ons/holds — done 2026-08-22 (~00:30), not yet pushed.** User supplied a separately-built UI prototype (Manus-generated, zero real backend, all data hardcoded) with a "Quiet Japanese Editorial" design (vermilion/warm-paper/charcoal, DM Sans + Noto Serif JP). Fully ported the visual design and interaction patterns onto `apps/customer-menu`'s real backend — customer-menu now has its own distinct look from dashboard-web/staff-clock's red/black/gold theme, per user decision. Also built (for real, not cosmetic, per user decision) the two features the prototype had: ingredient-hold customization (sourced from real `recipe_items`) and add-ons (new `menu_addons`/`digital_order_addons` schema, migration `0017`, applied live, seeded with the prototype's own 5 add-ons). New endpoints `GET /public/addons`, `GET /public/product-sizes/{id}/recipe`; approval now folds add-ons into the resulting transaction's total/tax. `PendingOrders.tsx` shows held ingredients + add-ons. Verified live: 13/13 automated checks + a full Playwright walkthrough (real ingredients checked as held, receipt-styled confirmation correctly polling pending → confirmed with the real dish name shown), zero console errors. See `SESSION_HANDOFF.md` "🎌 Digital menu visual redesign" section for full detail. Not yet pushed/deployed.
