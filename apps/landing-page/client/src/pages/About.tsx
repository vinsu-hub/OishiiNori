...
export default function About() {
  ...
  return <div className="on-site"><SiteNav /><main id="main-content">
    <section className="on-catering-hero on-container on-about-hero"><p className="on-label">OISHII NORI / OUR STORY</p><h1>MADE FOR<br /><span>GOOD CRAVINGS.</span></h1><p lang="ja" className="on-intro-japanese">おいしいのり</p><p>At Oishii Nori, we serve more than just sushi. We serve good vibes, fresh ingredients, and bold flavors — all in a space made for great food and even better company.</p><div className="on-actions"><a className="on-button" href="/menu">Order now ↗</a><a className="on-button on-outline" href="/menu?reserve=1">Reserve ↗</a></div></section>
    <section className="on-serve-strip"><div className="on-container"><p className="on-eyebrow">WHAT WE SERVE</p><div>{['Sushi', 'Ramen', 'Katsu', 'Sushi Boats'].map(name => <span key={name}>{name}</span>)}</div><a href="/our-menu">Explore our menu ↗</a></div></section>
    ...
    <section className="on-about-find on-container"><div><p className="on-eyebrow">FIND US</p><h2>YOUR TABLE<br />IS WAITING.</h2><p>Pedro Guevara Ave., Santa Cruz, Laguna 4009</p><p className="on-hours">Kitchen hours · {hoursLabel}</p><a className="on-map-link" href="https://maps.app.goo.gl/9oACBZo6tUdzyabK7" target="_blank" rel="noreferrer">Open in Maps ↗</a></div><div className="on-about-gather"><p lang="ja">みんなで</p><h2>BRING YOUR<br />GOOD COMPANY.</h2><p>Explore catering for your next gathering.</p><a className="on-button on-light" href="/catering">Catering ↗</a></div></section>
