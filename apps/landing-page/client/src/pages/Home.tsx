...
export default function Home({ initialData }: { initialData?: HomeInitialData } = {}) {
  ...
  return <div className="on-site"><SiteNav home /><main id="main-content">
    <section id="home" className="on-hero"><div className="on-hero-copy"><p className="on-eyebrow">EXCLUSIVE ROLLS</p><h1>OISHII<br /><span>NORI</span></h1><p className="on-tagline" lang="ja">おいしいのり</p><p>Rolls with a little more feeling.<br />Fresh ingredients, bright ideas,<br />and a table waiting for you.</p><div className="on-actions"><a className="on-button" href="/menu">Order now →</a><a className="on-button on-outline" href="/menu/?reserve=1">Reserve →</a></div></div><div className="on-hero-art"><div className="on-brush" aria-hidden="true" /><FoodPhoto src="/hero/sushi-boats.jpg" alt="Oishii Nori wooden sushi boats filled with maki and carved vegetable flowers" eager /><span className="on-japanese" lang="ja">新鮮な素材・最高の味</span></div></section>
    <section id="about" className="on-intro on-container"><div><p className="on-intro-japanese" lang="ja">おいしいのり</p><h2>MADE FOR<br />GOOD CRAVINGS.</h2></div><p>At Oishii Nori, we serve more than just sushi.<br />We serve good vibes, fresh ingredients,<br />and bold flavors — all in a space made<br />for great food and even better company.</p><span className="on-seal" lang="ja">美味</span></section>
    <section id="menu" className="on-sellers"><div className="on-container on-sellers-layout"><div className="on-section-title"><p className="on-eyebrow">ON THE TABLE</p><h2>BEST<br />SELLERS</h2><a href="/menu">Explore the menu →</a></div><p className="on-sellers-tagline">The crowd favorites.<br />Always a good idea.</p><div className="on-seller-grid">{BEST_SELLERS.map(item => <a href="/menu" className="on-seller" key={item.name}><div className="on-seller-image"><FoodPhoto src={item.photo} alt={item.name === 'Oishii Nori Ramen' ? 'Chashu ramen with golden broth, pork, egg and scallions' : item.name === 'Torikatsu Maki' ? 'Illustrative Torikatsu Maki with crispy potato strings (AI-generated)' : 'Real Oishii Nori sushi boats with assorted maki'} /><span className="on-badge">BEST<br />SELLER</span></div><div className="on-seller-copy"><h3>{item.name}</h3><p>{item.description}</p>{price(item.name) ? <strong className="on-price">{price(item.name)}</strong> : <span className="on-price-fallback">View menu for prices →</span>}</div></a>)}</div></div></section>
    <section className="on-categories"><div className="on-container"><div className="on-category-heading"><p className="on-eyebrow">MORE THAN SUSHI.</p><h2>SUSHI. RAMEN. KATSU.<br />YOUR KIND OF <span>OISHII.</span></h2></div><div className="on-category-grid">{Object.entries(CATEGORY_PHOTOS).map(([name, src]) => <a href="/menu" key={name}><FoodPhoto src={src} alt={name === 'Sushi' ? 'Illustrative Torikatsu Maki (AI-generated)' : name === 'Drinks' ? 'Illustrative Oishii Nori milk tea cups (AI-generated)' : name === 'Ramen' ? 'Chashu ramen with golden broth' : 'Cheesy katsu served with rice'} /><h3>{name} <span>→</span></h3></a>)}</div></div></section>
    <section className="on-experience"><FoodPhoto src={INTERIOR_PHOTO} alt="A real Oishii Nori sushi boat spread ready to share" /><div className="on-experience-heading"><p className="on-eyebrow">THE OISHII NORI EXPERIENCE</p><h2>YOUR TABLE<br />IS WAITING.</h2></div><div className="on-experience-copy"><p>From casual rolls to late-night cravings,<br />Oishii Nori is made for good food,<br />good company, and a table worth staying at.</p><a href="#find-us">View branches →</a></div><span className="on-experience-japanese" lang="ja">一緒に食べよう</span></section>
    ...
    <section id="find-us" className="on-find on-container"><div className="on-map-panel"><p className="on-eyebrow">FIND US</p><h2>OISHII NORI<br /><span>SANTA CRUZ</span></h2><p className="on-address"><span aria-hidden="true">●</span> Pedro Guevara Ave.<br />Santa Cruz, Laguna 4009, Philippines</p><p className="on-hours">Kitchen hours · {hoursLabel}</p><MapView className="on-map" {...shopLocation} zoom={17} /><a className="on-map-link" href="https://maps.app.goo.gl/9oACBZo6tUdzyabK7" target="_blank" rel="noreferrer">Open in Maps ↗</a></div><div id="contact"><h2>CONTACT US.</h2><p>A question, a craving, or something to share? Drop us a line.</p><InquiryForm kind="contact" /></div></section>
