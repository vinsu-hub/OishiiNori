...
export default function Home({ initialData }: { initialData?: HomeInitialData } = {}) {
  ...
  return <Router ssrPath="/"><div className="on-site"><SiteNav home /><main id="main-content">
    ...
    <section className="on-intro on-container"><div><p className="on-intro-japanese" lang="ja">おいしいのり</p><h2>MADE FOR<br />GOOD CRAVINGS.</h2></div><p>At Oishii Nori, we serve more than just sushi.<br />We serve good vibes, fresh ingredients,<br />and bold flavors — all in a space made<br />for great food and even better company.<br /><a className="on-story-link" href="/about">Read our story ↗</a></p><span className="on-seal" lang="ja">美味</span></section>
    <section className="on-sellers"><div className="on-container on-sellers-layout"><div className="on-section-title"><p className="on-eyebrow">ON THE TABLE</p><h2>BEST<br />SELLERS</h2><a href="/our-menu">See the full menu ↗</a></div><p className="on-sellers-tagline">The crowd favorites.<br />Always a good idea.</p><div className="on-seller-grid">{BEST_SELLERS.map(item => <a href="/menu" className="on-seller" key={item.name}><div className="on-seller-image"><FoodPhoto src={item.photo} alt={item.name === 'Oishii Nori Ramen' ? 'Chashu ramen with golden broth, pork, egg and scallions' : item.name === 'Torikatsu Maki' ? 'Illustrative Torikatsu Maki with crispy potato strings (AI-generated)' : 'Real Oishii Nori sushi boats with assorted maki'} /><span className="on-badge">BEST<br />SELLER</span></div><div className="on-seller-copy"><h3>{item.name}</h3><p>{item.description}</p>{price(item.name) ? <strong className="on-price">{price(item.name)}</strong> : <span className="on-price-fallback">View menu for prices →</span>}</div></a>)}</div></div></section>
