...
export default function SiteNav({ home = false }: { home?: boolean }) {
  ...
  return <header className="on-header"><a className="on-skip" href="#main-content">Skip to content</a><div className="on-nav">
    ...
    <nav id="site-navigation" className={`on-links ${open ? 'is-open' : ''}`} aria-label="Main navigation">{links.map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)}>{label}</a>)}</nav>
    <a className="on-logo" href={anchor('home')} aria-label="Oishii Nori home"><img src="/logo.jpg" alt="Oishii Nori" width="64" height="64" /></a>
