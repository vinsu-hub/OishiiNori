...
**POS Terminal** is the checkout screen: a product/size grid with large tablet-friendly category pills, a cart, discount selection, a VAT/Non-VAT toggle, and a Charge button. To hold an ingredient on a line ("no cucumber"), open the order editor and check the box for it — that ingredient is excluded from automatic stock deduction entirely, not deducted-then-restored. For a Dine In order the cashier enters a table number via a high-visibility red "Pick a table" dropdown; if that table has a confirmed reservation in its window (3.15) it shows as **Reserved** and Charge is blocked until a manager clears it with a PIN override, and a party over the table's capacity is rejected. Choosing **Card** as the payment method prompts for Debit or Credit before it's accepted, and that choice is recorded on the sale. The **VAT/Non-VAT** toggle is independent of any discount's own VAT-exempt flag — it lets a cashier book an order non-VAT even with no VAT-exempt discount applied; a VAT-exempt discount still zeroes tax regardless of the toggle's position. Other POS Terminal features: **Held Orders** (park a cart mid-sale, F4, resumes later from the same shift, sessionStorage-scoped), **Favorites** (star items, persists across sessions), an **upsell rail** suggesting up to 6 not-in-cart drinks/dessert/light-side items, an **Owner's Request** flow (a manager/executive PIN re-verification for actions like a manual discount override), and **offline resilience** — if a Charge fails because the device genuinely lost network (not a real rejection like insufficient stock), the sale queues locally in the browser and shows "Offline — order queued, will sync automatically," auto-flushing once the connection returns; the header's sync badge reflects real pending-sync count. A **Start/End Business Day** control sits in the header beside the sync indicator — POS is locked until the cashier's own kiosk credentials confirm the current calendar day is started, and closing it (also credential-confirmed) is a one-way action for that day; once closed, that date can't be reopened. Executives are exempt from this lock (both the overlay and the server-side check) and can use POS regardless of the day's state.
