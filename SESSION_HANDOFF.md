...
**Date:** 2026-08-20 (build session) · **Updated:** 2026-08-24 (this update adds five new pieces on top of the 2026-08-22 close-out: **deployed and live in production** — Menu Editing (executive product/recipe/image CRUD), the P&L dashboard (automatic COGS from existing recipe/BOM data), and editable ingredient unit cost; **built and verified locally, not yet deployed** — the physical stock count tool (4-station digitization of the client's real handwritten stock sheets) and admin-editable measurements (full ingredient-field editing incl. base_unit, plus a configurable VAT rate replacing two hardcoded duplicates). See each feature's own section below for full detail.)
**Repo:** `D:\ioshinori\oishii-nori-command-suite` — pushed to GitHub: `https://github.com/vinsu-hub/OishiiNori` (private, `main` branch). **Not everything below is pushed** — the physical stock count tool and admin-editable-measurements sections (both 2026-08-24) are committed to git history only in the sense of being ready; as of this write-up they are still uncommitted local changes (`git status` shows them modified/untracked). Everything else in this document is committed and pushed.
...
**Build status: functionally complete, everything verified (live or locally).** All 4 apps deployed to production; two 2026-08-24 features (stock count tool, admin-editable measurements) are verified against local dev only and awaiting a deploy decision — see "To continue" below for exactly what's pending. The two long-standing blockers from this project's earlier history — the Supabase `hr` schema not being exposed to PostgREST, and Oishii AI's LLM provider having no working billing — were both resolved on 2026-08-22 and remain resolved (confirmed again this update: `GET /hr/holidays`, `/attendance`, `/payroll`, `/employees` all 200 live; Oishii AI answers real questions correctly through the live chat UI on production). No other open items remain beyond the pending-deploy decision noted above.
...
**Verified live end-to-end** on local dev: spot-checked auto-links (Sriracha, Japanese Rice (raw)→Sushi rice (raw)), confirmed none of the chiller/freezer/nori/mayo groups got linked, recorded a count for a linked item and confirmed it drove a real `count_adjustment` movement + a `delivery` movement with `reference_id=null` (no FK violation) on the underlying ingredient, recorded a count for an unlinked item and confirmed only `stock_items.current_stock` changed, confirmed same-day resubmission updates in place (no duplicate entries), confirmed role-gating (employee 403 on catalog mutation, 200 on day-to-day counting), and a real Playwright walkthrough (station tabs, VERIFY/linked badges, the Manage Catalog edit dialog actually linking an ingredient and persisting) with zero console errors throughout. All test data reverted afterward. Frontend `tsc --noEmit` clean.
