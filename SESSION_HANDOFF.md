...
**Build status: functionally complete, everything verified live.** All 4 apps deployed to production. The one long-standing blocker — the Supabase `hr` schema not being exposed to PostgREST — **is resolved** (confirmed repeatedly this session: `GET /hr/holidays`, `/attendance`, `/payroll`, `/employees` all 200 live). HR/Payroll is fully live-verified, including real generated payroll runs, PDF payslips, and demo data. **Oishii AI is also now fully live** — the originally-configured xAI account had no billing, so it was switched to Groq (free tier); after also trimming the context payload to fit Groq's per-organization TPM cap, real questions (revenue, best seller, loss drivers, an individual employee's pay rate, ingredient stock) all return correct, live-data-grounded answers through the actual chat UI on production. No open items remain.
